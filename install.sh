...
wget -q -O /usr/local/bin/wireproxy https://github.com/Dmr255/sccvpn/raw/main/wireproxy
...
XRAY_CONFIG=raw.githubusercontent.com/Dmr255/sccvpn/main/config
...
wget -q -O /usr/local/etc/xray/config/04_inbounds.json "https://${XRAY_CONFIG}/04_inbounds.json"
XRAY_UUID="$uuid" XRAY_PWTR="$pwtr" XRAY_PWSS="$pwss" XRAY_SERVERPSK="$serverpsk" XRAY_USERPSK="$userpsk" perl -0pi -e 's/\$uuid/$ENV{XRAY_UUID}/g; s/\$pwtr/$ENV{XRAY_PWTR}/g; s/\$pwss/$ENV{XRAY_PWSS}/g; s/\$userpsk/$ENV{XRAY_USERPSK}/g; s/\$\(cat \/usr\/local\/etc\/xray\/serverpsk\)/$ENV{XRAY_SERVERPSK}/g' /usr/local/etc/xray/config/04_inbounds.json
...
wget -q -O /var/www/html/index.html https://raw.githubusercontent.com/Dmr255/sccvpn/main/index.html
wget -q -O /etc/nginx/nginx.conf https://raw.githubusercontent.com/Dmr255/sccvpn/main/nginx.conf
domain=$(cat /usr/local/etc/xray/dns/domain)
...
apt update -y
apt install -y openssh-server dropbear sstpd stunnel4 git
...
systemctl enable sstpd 2>/dev/null || true
...
cd /usr/bin
GITHUB=raw.githubusercontent.com/Dmr255/sccvpn/main/
...
wget -q -O ssh-ctl "https://${GITHUB}/service/ssh-ctl.sh"
wget -q -O sstp-ctl "https://${GITHUB}/service/sstp-ctl.sh"
wget -q -O zivpn-ctl "https://${GITHUB}/service/zivpn-ctl.sh"
